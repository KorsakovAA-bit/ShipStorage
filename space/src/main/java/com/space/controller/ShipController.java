...
public class ShipController {
        ...
        for (int i = 0; i < ships.size(); i++) {
            ...
            if (isUsed != null) {
                try {
                    ...
                    System.out.println("Параметр \"isUsed\" не boolean");
                }
            }
            if (minSpeed != null) {
                try {
                    ...
                    System.out.println("Параметр \"minSpeed\" не double");
                }
            }
            if (maxSpeed != null) {
                try {
                    ...
                    System.out.println("Параметр \"maxSpeed\" не double");
                }
            }
            if (minCrewSize != null) {
                try {
                    ...
                    System.out.println("Параметр \"minCrewSize\" не int");
                }
            }
            if (maxCrewSize != null) {
                try {
                    ...
                    System.out.println("Параметр \"maxCrewSize\" не int");
                }
            }
            if (minRating != null) {
                try {
                    ...
                    System.out.println("Параметр \"minRating\" не double");
                }
            }
            if (maxRating != null) {
                try {
                    ...
                    System.out.println("Параметр \"maxRating\" не double");
                }
            }
        }
            ...
            ships = getPages(ships, pageNumber, pageSize);
    ...
    public ResponseEntity<Ship> updateShipById(@PathVariable("id") String strShipId, @RequestBody Ship enterShip) {
        ...
        if(enterShip.getName() != null){
            if(enterShip.getName().length() <= 50 && enterShip.getName().length()>0) currentShip.setName(enterShip.getName());
            else return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }

        if(enterShip.getPlanet() != null){
            if(enterShip.getPlanet().length() <= 50) currentShip.setPlanet(enterShip.getPlanet());
            else return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        ...
        if(enterShip.getSpeed() != null) {
            if(enterShip.getSpeed() >= 0.01 && enterShip.getSpeed() <= 0.99) {
                DecimalFormat decimalFormat = new DecimalFormat("#.##");
        ...
        if(enterShip.getCrewSize() != null) {
            if(enterShip.getCrewSize() >= 1 && enterShip.getCrewSize() <= 9999) currentShip.setCrewSize(enterShip.getCrewSize());
            else return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
    ...
    private List<Ship> getPages(List<Ship> fullList, String pageNumber, String pageSize){
        try {
            int pgNumber = Integer.parseInt(pageNumber);
            int pgSize = Integer.parseInt(pageSize);
            int numberOfPage = fullList.size() % pgSize == 0 ? (fullList.size() / pgSize) : (fullList.size() / pgSize) + 1;
            if(pgNumber > numberOfPage - 1) pgNumber = 0;
            int beginWith = pgNumber * pgSize > fullList.size()-1 ? fullList.size()-1 : pgNumber * pgSize;
            int stopWith = beginWith + pgSize > fullList.size() ? fullList.size() : beginWith + pgSize;
            return fullList.subList(beginWith, stopWith);
        } catch (NumberFormatException e){
            e.printStackTrace();
        }
        return fullList;
